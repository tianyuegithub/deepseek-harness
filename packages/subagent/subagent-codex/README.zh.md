...
Codex 子级会在一个全新的临时线程中，以单个轮次接收这些独立文本块。提供受信单次运行 cwd 时使用该工作区，否则使用父 Session cwd；所选提供方实例会固定已配置的模型、环境、非交互审批策略与沙箱模式，而省略的模型及其余产品设置来自 Codex 原生配置。可执行版本来自 Bundle 锁定的平台载荷。

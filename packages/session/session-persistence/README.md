...
A stored log the current build cannot faithfully interpret is refused with a direction-aware error, never misread. `SESSION_FORMAT_VERSION` remains v0 and this build provides no format-migration path; a newer version instructs the operator to upgrade the harness. The decoder accepts only the bounded same-version record variants named below. Every event type unknown to core refuses reconstruction unless an earlier `session/external-event-producer` declaration and an exact active producer registration admit it; removing that producer preserves the artifact but makes its history unsupported until a matching registration returns. Committed-prefix corruption rejects as `SessionPersistenceCorruptionError` ([base rationale](../../../.agents/notes/implemented/simplification/2026-08-25-fail-closed-session-event-vocabulary.md), [external contract](../../../.agents/notes/implemented/architecture/2026-08-29-durable-external-session-event-producers.md)). A `load` on an id still bound to a live session first flushes its snapshot and rejects while its turn is open; a cold load applies recovery.
